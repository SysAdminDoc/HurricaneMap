...
| Q2 | Centralized app state and hash parsing | Filter state, URL state, and validation live outside `main.js` with focused tests. | [ ] | Hash recovery was hardened in v1.3.9. | Extract a `url-state` module and unit-test malformed/default hashes. |
...
- [ ] Extract URL hash decode/encode/validation from `src/main.js`.
- [ ] Add focused tests for default hash omission, malformed percent encoding, invalid categories, invalid states, and open storm/state restoration.
