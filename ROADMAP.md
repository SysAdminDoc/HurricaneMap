...
- [x] **P10.2 NHC cone of uncertainty render** — ✅ v1.3.9. Wires active storms to the official Esri/NHC active-hurricane GeoJSON layers for Forecast Error Cone, Forecast Track, and Observed Track; caches GIS geometry for 6 hours unless the advisory identity changes; renders the semi-transparent cone plus official forecast/observed tracks behind the live marker with a settings toggle.
...
- [x] **P13.2 Wind-field swath 3D cone** — ✅ v1.3.9. Focused or single-storm 3D globe views now render HURDAT2 wind-radii records as translucent asymmetric cone fans for 34/50/64 kt thresholds. The cone footprint uses the four quadrant radii at each track point, the apex rises with storm wind speed, outlines preserve the threshold footprint, and a topbar Wind cones control disables itself for storms without 2004+ radii data. Covered by utility tests and the Cesium Playwright smoke.
- [x] **P13.3 Population impact overlay** — ✅ v1.3.9. Storm panels for modern landfalling storms now show an "Est. exposure" metric, headed by Cat-2+ winds and backed by Cat-1/2/3/5 estimates. The estimator processes HURDAT2 wind-radii records near each U.S. landfall, derives conservative inner-core Cat-2/3/5 footprints from the 64 kt radii where gridded maxima are unavailable, and applies the bundled `data/us-states.geojson` density attributes with explicit screening-estimate methodology copy instead of pretending to ship a full LandScan raster bundle.
...
- [x] **P14.2 Offline-first service worker v2** — ✅ v1.3.9. Historical lookup datasets (`landfalls`, `storms`, `stats`, metadata, impacts, glossary, storm events, state density GeoJSON, raw HURDAT2 text, and radar manifest) now preinstall through the service worker into compressed IndexedDB with CacheStorage fallback and stale-while-revalidate refresh. Local radar PNGs are intentionally cache-first on demand rather than install-time preloaded so the large committed radar archive degrades gracefully without creating a massive first-install quota hit.
- [x] **P14.3 Bundle size audit & tree-shaking** — ✅ v1.3.9. Added an esbuild-powered bundle audit to `npm run build` with a hard 100 KB gzip budget for initial JavaScript; current audited initial JS is 24.8 KB gzip. Non-critical surfaces now load through dynamic imports, including storm details/radar/animation/chart stack, stats, comparison, state deep-dive, active-storm overlays, population/surge overlays, exports/reports/QGIS, glossary, keyboard palette, onboarding, and the 3D globe.
- [x] **P14.4 GitHub Pages CDN optimization** — ✅ v1.3.9. Added a deployment-ready Cloudflare Worker that proxies the GitHub Pages origin with route-aware `Cache-Control`, `CDN-Cache-Control`, and `Cloudflare-CDN-Cache-Control` headers, short HTML TTLs, stronger app/data edge caching, immutable image/radar caching, image format/quality hints for non-radar images, compression-safe `Vary: Accept-Encoding`, and CDN hit/miss timing markers. Added Wrangler config, deployment/measurement docs, and policy tests; live global latency measurement remains the post-deploy validation step because it requires the Cloudflare route/zone credentials.
- [x] **P14.5 Docker + self-hosted option** — ✅ v1.3.9. Added a `python:3.12-alpine` Dockerfile that serves the static app on port 8080 with Python `http.server`, runs as a non-root `hurricanemap` user, and healthchecks `/data/metadata.json`. Added `.dockerignore`, self-hosting documentation, and a Dockerfile contract test so institutional/intranet deployments can package the full committed data bundle, including local radar frames, without carrying development artifacts.
