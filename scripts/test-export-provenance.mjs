...
function assertProvenance(provenance, label) {
  ...
  assert.equal(provenance.app_version, '1.11.0', `${label}: app version`);
  ...
  assert.equal(provenance.data_release.coverage.catalog.storm_count, 587, `${label}: coverage storm count`);
  assert.equal(provenance.data_release.coverage.datasets.find(dataset => dataset.id === 'radar-archive').availability.frames, 1697, `${label}: radar coverage`);
