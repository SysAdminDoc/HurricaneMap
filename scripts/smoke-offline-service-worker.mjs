...
try {
  ...
  const releaseTuple = await page.evaluate(async () => {
    ...
    if (tuple.data_cache !== dataCacheName || tuple.shell_cache !== 'hm-shell-hm-v1.11.1') {
  ...
  assert(offlineResult.catalogStorms === 587 && offlineResult.radarFrames === 1697, 'offline archive coverage facts are unavailable');
