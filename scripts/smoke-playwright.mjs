...
// at 1024px and again at 1378px the header read "...Atlas · 587 st", cut
 ...
 * blank map under "0 of 750" with no message and no way back, which reads as a
    ...
    try {
      ...
      const expected = await page.evaluate(async () => {
        ...
        return {
          ...
          tableCount: t('table.countMany', (750).toLocaleString(getLocale())),
...
try {
  ...
  assert(/587\s+storms/.test(provenanceText), 'About provenance did not render the storm count.');
  assert(/750\s+landfalls/.test(provenanceText), 'About provenance did not render the landfall count.');
  ...
  assert(poster.stormCount === 583 && poster.segmentCount > 10000, `poster did not honor the 583 drawable tracks in the unfiltered storm set: ${JSON.stringify(poster)}`);
  assert(poster.colorCount > 30, `poster canvas lacks rendered visual variation: ${JSON.stringify(poster)}`);
  assert(/NOAA\/NHC HURDAT2/.test(poster.attribution) && /583 storms/.test(poster.label), `poster metadata is incomplete: ${JSON.stringify(poster)}`);
  ...
  await page.waitForFunction(() => {
    ...
    return /242 of 587/.test(host?.textContent || '') &&
