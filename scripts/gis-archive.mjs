...
export function parseValidTime(validtime, issuedIso) {
  ...
  const candidate = new Date(Date.UTC(
    issued.getUTCFullYear(), issued.getUTCMonth(), Number(dd), Number(hh), Number(mi),
  ));
  if (candidate.getTime() < issued.getTime() - DAY_MS) candidate.setUTCMonth(candidate.getUTCMonth() + 1);
  else if (candidate.getTime() > issued.getTime() + 9 * DAY_MS) candidate.setUTCMonth(candidate.getUTCMonth() - 1);
  return {
...
// A cone ring is about 350 points as published. Reduced at this tolerance it is
// about 86 and departs from the published outline by at most 1.19 km, which on
// a five-day cone some 370 km across is three parts in a thousand. Rounding the
// coordinates to two decimals alone already costs 0.63 km, so the simplification
// is close to free; the full rings would be 3.9 MB for the era rather than 0.7.
export const CONE_SIMPLIFY_TOLERANCE_DEGREES = 0.01;
export const CONE_COORDINATE_DECIMALS = 2;
...
export function readAdvisoryArchive(buffer, { stormId, label = 'advisory' } = {}) {
  ...
  for (const row of points.rows) {
    ...
    if (!Number.isFinite(tau) || tau === 9999 || tau < 0) continue;
    let valid;
    try {
  ...
  let initial = null;
  let best = -1;
  for (const [origin, count] of votes) {
    // Ties go to the earlier origin, so the same file always reads the same way.
    if (count > best || (count === best && origin < initial)) { best = count; initial = origin; }
  }
  const initialIso = new Date(initial).toISOString().replace('.000', '');
  // One dissenting row is the current position and is expected. More than that
  // means the two clocks in the file disagree and the origin is a guess.
  if (best < points.rows.length - 1 && best < points.rows.length * 0.6) {
    throw new Error(`${label}: only ${best} of ${points.rows.length} rows agree on a forecast origin`);
  }
  ...
  for (const row of points.rows) {
    ...
    // TCSPD for every row past the first.
    const wind = Number(row.MAXWIND);
