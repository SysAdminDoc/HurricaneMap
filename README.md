...
The app uses an offline-first service worker. Runtime historical lookup data is preinstalled into compressed IndexedDB with CacheStorage fallback, while large local radar PNGs and the raw HURDAT2/release-manifest source bundle are cached only after an explicit user action. Each launch verifies the shell/data release tuple and labels it intact, evicted, stale-but-valid, or invalid; diagnostics offers one-click repair, and storm panels report how many radar frames are cached. Settings reports browser usage/quota, protects the core shell/data scopes, and exposes a 13 MB cap for the optional source bundle alongside independent tile/radar cleanup; an opened radar timeline can save an explicit, bounded per-storm offline pack. When shell or offline-data assets change, bump `SW_VERSION` in `sw.js`; installed users will then see an in-app reload prompt instead of silently staying on stale UI. The worker registers as a module and retries as a classic script; that second attempt is the classic service worker for the Firefox ESR 140 line, which predates the release that brought module workers to Baseline. It costs an extra full offline run on every test, and `scripts/baseline-contract.mjs` carries a dated plan for removing it, not before 2027-01-01 and not before the pinned-enterprise tail has rolled. `npm run check:baseline` fails if that plan and the code disagree about whether the fallback is still here. On iPhone and iPad the storage panel adds what the documentation says about Safari's seven-day eviction window, and points at the Home Screen install as the way around it. That note is labelled as unconfirmed, because MDN describes the rule and Apple's own write-up does not, and nobody has yet watched it happen on a device.
...
- **Potential storm surge flooding**: an opt-in layer draws NHC's inundation footprint from the tropical weather summary MapServer. The legend leads with what the number means, a 10 percent chance the water goes above the published level, because that is the part people read wrong and a shaded coast without it invites the wrong reading. It carries its own toggle rather than riding the forecast cone's, and out of season the service returns no features, which the layer reports as empty rather than as a failure.
- **Track colour encoding**: tracks are coloured by Saffir-Simpson category by default, and a setting switches them to sustained wind, minimum pressure, or month of the season, with a legend naming the variable and its bands. The choice travels in the URL fragment. All three share one ramp because only one part of the colour space has room: every theme here draws a light basemap, so a track has to be dark to clear the 3:1 WCAG asks of a graphical object, and greens, cyans and golds run out of lightness long before the blue-to-red corridor does. Storms with no recorded pressure, and storms outside June to November, get their own colour instead of being pushed into the nearest band. `npm run check:track-ramps` measures every stop against every basemap fill in every mode and checks the stops stay apart under simulated protanopia, deuteranopia and tritanopia.
...
- **🚨 Active storm tracking**: when NHC reports active storms, a pulsing badge appears with the official cone/track, Potential Tropical Cyclone support, advisory/discussion/name-pronunciation/rip-current links, an optional GOES backdrop, hourly feed checks, and retry/backoff status. No edge worker is needed. Where one is deployed the app reads `CurrentStorms.json` through it, and everywhere else it reads the same storms from NHC's CORS-open tropical weather summary MapServer, which carries everything except the advisory and discussion URLs.
...
- **❎ NHC tropical outlook + marine warnings**: official formation disturbances render with the NHC's 2026 gray-X treatment for near-0% systems; an opt-in layer adds the offshore wind-warning zones for either of the two forecast bands NHC publishes, 0-24 hour or 24-48 hour, with the band named in the legend. The outlook falls back to the same summary MapServer when no relay is present, which costs the per-disturbance discussion paragraph and nothing else.
...
| --- | --- | --- | --- |
| `core` | [`hurricanemap-1.10.0-core.tar.gz`](https://github.com/SysAdminDoc/HurricaneMap/releases/download/v1.10.0/hurricanemap-1.10.0-core.tar.gz) (6.1 MB) | 25.3 MB | The whole historical atlas: 587 storms, 750 landfalls, every panel and export |
...
Runtime mapping code is deliberately pinned: Leaflet 1.9.4 is vendored locally for offline use, while Cesium 1.145 is loaded only for the optional globe with exact script and stylesheet integrity hashes. The disputed Leaflet advisory record (with its `check:popup-sinks` compensating control) and Cesium release/SRI decision are recorded in [`security/dependency-security-policy.json`](security/dependency-security-policy.json). Updating either requires checking its upstream license/release, changing the complete pinned asset pair, updating that decision record, and passing the desktop/mobile map and globe smokes.
...
Every storm also has a page of its own under `storms/`, generated by `npm run generate:storm-pages`: a readable six-hourly track table, its U.S. landfalls, the HURDAT2 revision behind them, a copyable APA and BibTeX citation, and a link into the map at that storm. They carry no JavaScript, so the content is there for a crawler, a reader without scripts, and a screen reader that should not have to drive a map to read a track. `storms/` indexes all 587 and the sitemap lists them, alongside a page per season under `seasons/`, per decade under `decades/` and per state under `states/`, each listing its storms and linked back from every storm it lists. There is a matching page per coastal city under `cities/`, from `npm run generate:city-pages`: how often a hurricane has passed within 50 km, the full chronological record those intervals were measured from, and a link into the map scoped to that city's state and years. Each storm page carries a 1200x630 social card of its own track, under `social/`, from `npm run generate:social-images`; that one needs Chromium, because Open Graph and Twitter both reject SVG. The cards are not byte-reproducible, so `social/manifest.json` records the hash of each card and of the drawing it came from, and the gate fails on a card that is stale, swapped or edited. `npm run test:storm-pages` rebuilds the pages in memory and fails if the result differs from what is checked in, is not byte-reproducible, if two city pages describe themselves the same way, or if two storm pages point at the same social card.
...
The static [`data/stac/catalog.json`](data/stac/catalog.json) exposes HURDAT2 tracks/landfalls and archived NEXRAD frames as a standards-based [STAC 1.1.0](https://github.com/radiantearth/stac-spec) catalog. Its HURDAT2 and radar collection summaries repeat the canonical archive year, basin, count, and value-status facts from `data/coverage.json`. The application entry point advertises it with an `application/json` alternate link in `index.html`, and the same relative entry point is included in both the core and full releases. Collection and item links are relative apart from `self`, so the catalog works from a checked-out repository, a core release, or a full release without a server. Each asset records its source URL/date, public-domain license, release availability, byte count, and SHA-256; radar PNG assets are marked `full` while their spatially indexed metadata remains in `core`. Open it in [STAC Browser](https://browser.moregeo.it/external/sysadmindoc.github.io/HurricaneMap/data/stac/catalog.json) to read it without a client, which the About dialog links too. Every document's `self` link is its own published address, because that is what a viewer resolves the rest of the tree from; every other link stays relative, so the catalog still navigates from a checked-out repository or an unpacked release with no server. Run `npm run check:stac` to validate navigation, geometry, manifest coverage, and asset checksums; regenerate coverage with `npm run generate:coverage`, then regenerate the deterministic files with `npm run generate:stac` before regenerating the release manifest.
...
Then run `python -m notebook notebooks/analysis-starter.ipynb`. The setup cell emits the same APA and BibTeX release citation as the browser and exports. The release gate can execute the same notebook twice without network access, using disposable output directories, and verify the 587-storm, 750-landfall, 370-hurricane-strength, and release-provenance contract:
...
│   ├── land-mask.json          # North American coastline (was the storm over water?)
...
The build also ingests the [AOML detailed continental U.S. landfall table](https://www.aoml.noaa.gov/hrd/hurdat/UShurrs_detailed.html) as an independent ground-truth artifact, scored over every year the table gives a position for. That is 1851 to 2024: the current release matches 337 of 352 hurricane-strength continental reference rows against 362 detected landfalls, which is 93.1% precision and 95.7% recall, and leaves 15 reference rows unmatched. Precision and recall are also reported per decade, because an average across 174 years hides the era that is actually worst; the weakest is the 2020s at 83.3% recall, and no decade falls below 80%. Every unmatched reference row is listed in `data/aoml-landfalls.json` by storm id, date and category, so a disagreement can be looked at rather than taken on trust. A miss is not automatically a detection failure, since several are storms the atlas does record at a position or hour outside the 125 km / 12 hour matching window.

This check used to cover 1983-1990 only, sixteen rows at a perfect score, which said very little about whether the rest of the record is right. One caveat is worth stating plainly: AOML's table skips 1971-1982, which is most of HURDAT2's own 1971-1990 landfall marking gap and precisely the window the inferred-landfall pass exists to recover. Seven of the ten inferred hurricane-strength candidates fall in those years, so there is no reference to score them against; they are reported as unscoreable rather than counted as wrong.
...
2. **See [LICENSE.md](LICENSE.md) for:**
   ...
   - The dataset citation and a BibTeX entry for this repository
...
| Archived NHC advisories (2008-2024 U.S.-landfalling Atlantic storms) | From 2015, the [NHC product archive](https://www.nhc.noaa.gov/archive/), one forecast/advisory product per advisory. Before 2015, the [NHC GIS forecast archive](https://www.nhc.noaa.gov/gis/archive_forecast.php), which carries the cone polygon NHC published with each advisory. Regenerate `data/advisories.json` with `node scripts/build-advisories.mjs` |
