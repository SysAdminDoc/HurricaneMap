...
Runtime mapping code is deliberately pinned: Leaflet 1.9.4 is vendored locally for offline use, while Cesium 1.143 is loaded only for the optional globe with exact script and stylesheet integrity hashes. The reviewed Leaflet advisory exception and Cesium release/SRI decision are recorded in [`security/dependency-security-policy.json`](security/dependency-security-policy.json) with an absolute review expiry. Updating either requires checking its upstream license/release, changing the complete pinned asset pair, updating that decision record, and passing the desktop/mobile map and globe smokes.
