...
- **❎ NHC tropical outlook + marine warnings**: official formation disturbances render with the NHC's 2026 gray-X treatment for near-0% systems; an opt-in layer adds the 0-24 hour offshore wind-warning zones. The outlook falls back to the same summary MapServer when no relay is present, which costs the per-disturbance discussion paragraph and nothing else.
