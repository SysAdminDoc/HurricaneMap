...
- Comparison tracks are readable on the map. The four pin colours were doing three jobs at once, and one value cannot do all of them: the panel behind a chip flips with the theme, while the basemap under a track stays light in both, since the dark theme only takes the tiles to 0.86 brightness. Composited with the polyline's own 0.85 opacity over the usual OpenStreetMap fills, the light set measured 2.52:1 and the dark set 1.02:1, against the 3:1 WCAG asks of a graphical object. A comment claimed 4.28:1, which turned out to be one colour against one fill with the opacity left out. Tracks have their own deeper tokens now, 3.81:1 at worst, and `check:track-contrast` recomputes that from the tokens and the opacity as they are written in source rather than from a number somebody typed.
