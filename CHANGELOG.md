...
- The five similar-storms rows on every storm panel open the storm they name. They passed a landfall record to the panel, and a landfall record carries no storm id, so the panel looked up an undefined storm and showed its "record unavailable" state. It carries no name or year either, so the view history discarded the entry for having no year: clicking a row opened a storm and recorded nothing. The rows were also unreachable by keyboard, styled with a pointer cursor and nothing else, and they are buttons now.
...
- A saved view keeps its data-release pin. The release pin was treated as something to strip from a URL nobody had shaped, but a saved view of the default view is exactly that URL plus its pin, so restoring one dropped the citation the saved view existed to carry. Anything that arrived in the fragment stays in it, and the flag now clears when the fragment does, instead of latching for the rest of the session.
...
- The Spanish for "Low" in the rapid-intensification risk tile said "Bas", which is Haitian Creole. It read "Bas" beside "Alto" and "Medio".
