...
### Changed
- The six Python gates resolve an interpreter instead of assuming one. They called bare `python`, which does not exist on distributions shipping only `python3`, so a contributor outside Windows could not run the suite and got a shell "not recognized" message that named nothing. They now try `python3`, then `python`, then the `py -3` launcher, and a machine with none of them gets an error saying exactly what was looked for.
