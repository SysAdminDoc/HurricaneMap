...
- Map overlays now have one owner for their layer, their request and their feed state. Turning off the sea-surface temperature layer, or opening a different storm's high-water marks, aborts the request that was filling the old one instead of letting it finish and discard the result. A response that arrives after teardown can no longer put its layer back on the map.
