...
- Two light-theme controls stopped being unreadable. Pausing playback turned the primary button into a dark slab carrying the light theme's grey ink, at 1.71:1. The comparison table's column headers used the dark palette's pastels as text on a white panel, where the mauve measured 2.00:1 and was also the only thing tying a column to its track on the map. The button takes the theme's own elevated surface now, and the four pin colours are theme tokens: the dark set is unchanged, the light set is dark enough to read both on the panel and on the OpenStreetMap basemap, and high contrast gets a darker set again for its own AAA bar. Each header carries a swatch beside its label, and switching theme redraws the pinned tracks and the table together, which the fixed hexes never had to do because they were equally wrong in both.
