...
function renderResults(results, lat, lon) {
  ...
  spBody.innerHTML = `
    ...
    <div class="sp-controls">${radiusBtns}</div>
    ...
    <ul class="sp-list">
      ${results.map(r => `
        <li data-sid="${escapeHtml(r.storm_id)}" tabindex="0">
          ...
          <span class="sp-meta">${escapeHtml(categoryLabel(r.category))} · ${r.distance_mi} mi${r.wind_at_closest ? ` · ${r.wind_at_closest} kt` : ''}</span>
        </li>
      `).join('')}
    </ul>
  `;
